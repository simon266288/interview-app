...
import { getAllQuestions, getEffectiveQuestions } from '../services/effectiveQuestionBank';
...
onMounted(() => {
  ...
  if (mode === 'exam') {
    // exam：按当前 seed 的题库池抽取 20 题（仅非 card）
    const pool = getEffectiveQuestions({ mode: 'sample', seed: bankSettings.seed, samplePerModule: bankSettings.samplePerModule }).filter(
      q => q.type !== 'card'
    );
    const shuffled = [...pool].sort(() => 0.5 - Math.random());
    currentQuestions.value = shuffled.slice(0, 20);
  } else if (mode === 'card') {
    const pool = getAllQuestions().filter(q => q.type === 'card');
    if (moduleName) {
      currentQuestions.value = pool.filter(q => q.module === moduleName);
    } else {
      currentQuestions.value = pool;
    }
  } else if (mode === 'list') {
    ...
    currentQuestions.value = ids.map(id => all.find(q => q.id === id)).filter(Boolean) as Question[];
    currentIndex.value = typeof parsed.startIdx === 'number' ? parsed.startIdx : 0;
  } else if (moduleName) {
    // practice：按当前 seed 的有效题库，过滤出该 module 的非 card
    const pool = getEffectiveQuestions({ mode: 'sample', seed: bankSettings.seed, samplePerModule: bankSettings.samplePerModule }).filter(
      q => q.type !== 'card'
    );
    currentQuestions.value = pool.filter(q => q.module === moduleName);
...
const selectOption = (idx: number) => {
  ...
  if (!isExamMode.value) {
    ...
    if (isRight) store.addCompleted(currentQ.value);
    else store.addWrong(currentQ.value);
  }
};

const markCard = (known: boolean) => {
  if (known) store.addCompleted(currentQ.value);
  else store.addWrong(currentQ.value);
  setTimeout(() => {
...
const toggleFavorite = () => store.toggleFavorite(currentQ.value);
